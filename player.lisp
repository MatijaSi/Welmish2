...
(defmethod draw-stats ((player player) &optional (offset (cons 0 0)))
  (let ((health (combat:health player))
    ...
    (io:draw-string             "Damage   / Armour"                        0 2 offset)     
    (io:draw-string (format nil "Raw:   ~A / ~A"   (car raw)   (cdr raw))   0 3 offset)
    (io:draw-string             "Damage   / Resistance"                    0 5 offset)
    (io:draw-string (format nil "Fire:  ~A / ~A %" (car fire)  (cdr fire))  0 6 offset)
    (io:draw-string (format nil "Water: ~A / ~A %" (car water) (cdr water)) 0 7 offset)
